...
const CalendarManager = dynamic(
  ...
  {
    loading: () => (
      <div className="min-h-[50vh] w-full animate-pulse rounded-xl border border-slate-200 bg-white p-4 shadow-inner">
...
/** Parents see schedule activities only — games, practices, and general events (not injury timelines or coach-only entries). */
export function filterEventsForParentPortal(events: EventItem[]): EventItem[] {
  return events.filter((e) => {
    if (e.linkedInjuryId) return false
    const aud = String(e.audience || "").toLowerCase()
...
export function ParentPortalCalendar() {
  const { teamId } = useParentPortal()
  ...
  const events = useMemo(() => {
    const raw = mapApiToEventItems(calQ.data ?? [])
    return filterEventsForParentPortal(raw)
  }, [calQ.data])
  ...
  return (
    <div
      className="flex min-h-[min(720px,calc(100dvh-13rem))] w-full flex-col overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm"
