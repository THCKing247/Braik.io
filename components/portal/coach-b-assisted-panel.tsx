...
export function CoachBAssistedPanel({
  ...
  return (
    <div
      className={
        "rounded-xl border border-border bg-card overflow-hidden text-foreground " + (className ?? "")
      }
    >
      ...
      <div className="p-3 space-y-3">
        <div className="flex gap-2">
          ...
          />
          <Button size="sm" onClick={handleAsk} disabled={loading || !prompt.trim() || !canEdit} className="shrink-0 bg-primary text-primary-foreground hover:bg-primary/90 disabled:opacity-70">
                ...
                >
                  <div className="flex items-start justify-between gap-2">
                    <div className="min-w-0">
                      ...
                      <p className="text-xs text-muted-foreground mt-0.5">
                        {suggestion.conceptType}
                        {suggestion.concept && (
                          <span className="text-muted-foreground"> · {suggestion.concept}</span>
                        )}
                      </p>
                    </div>
                    <button
                  ...
                  {suggestion.tags && suggestion.tags.length > 0 && (
                    <div className="mt-2 flex flex-wrap gap-1">
                      {suggestion.tags.map((tag) => (
                        <span
                          key={tag}
                          className="inline-flex items-center rounded-md bg-muted/50 border border-border px-1.5 py-0.5 text-xs text-muted-foreground"
                        >
                          {tag}
                        </span>
                      ))}
                  ...
                  <div className="flex flex-wrap gap-2 mt-3">
                    <Button
                      size="sm"
                      variant="default"
                      className="h-7 text-xs bg-primary text-primary-foreground hover:bg-primary/90 disabled:opacity-70"
