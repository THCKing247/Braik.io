...
function getThreadDisplayName(thread: Thread, userId: string) {
...
function threadCategoryLabel(thread: Thread, userId: string) {
...
function primaryThreadPeer(thread: Thread, userId: string) {
  const others = thread.participants.filter((p) => p.user.id !== userId)
  if (others.length === 1) return others[0]
  return null
}

function avatarClassForKind(kind?: ParticipantKind) {
  switch (kind) {
    case "player":
...
const MessagingThreadInboxCard = memo(function MessagingThreadInboxCard({
  thread,
  userId,
  isSelected,
  isStarred,
  ...
  return (
    ...
    >
      ...
      >
        <Star
          className={cn("h-5 w-5", isStarred && "fill-amber-400 text-amber-500")}
            ...
            <>
              {sortedThreadSections.starred.length > 0 && (
                <div className="mb-1">
                  ...
                  {!priorityCollapsed &&
                    sortedThreadSections.starred.map((t) => (
                      <MessagingThreadInboxCard
                        key={t.id}
                        thread={t}
                        userId={userId}
              ...
              <div className="mt-1">
                ...
                {sortedThreadSections.rest.map((t) => (
                  <MessagingThreadInboxCard
                    key={t.id}
                    thread={t}
                    userId={userId}
