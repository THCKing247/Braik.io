...
-- Requires invite_code_type to exist (from 20260334000004_invite_codes.sql). Skip if type missing.

do $$
begin
  if exists (select 1 from pg_type t join pg_namespace n on n.oid = t.typnamespace where n.nspname = 'public' and t.typname = 'invite_code_type')
     and not exists (
